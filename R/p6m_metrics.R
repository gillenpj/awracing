...
#' Papers 2b and 3 on the ranking measures, beside their single-bet ROI
#'
#' The contrast the section turns on: three measures computed over every
#' runner-row or every race separate the two models' returns from noise well
#' enough to establish a tie, while the ROI difference on about 1,100 bets
#' does not. `n_basis` records what each measure is computed over, because
#' they are not all the same.
#'
#' @param boot_ranking The published 3-vs-2b ranking bootstrap.
#' @param ranking_metrics_2b Paper 2b's ranking metrics, for the row counts.
#' @param single_2b,single_3 The two single-bet win backtests.
#' @param roi_contrast The recomputed single-bet 3-2b contrast.
#' @return A tibble, one row per measure.
p6m_resolution_table <- function(boot_ranking, ranking_metrics_2b,
                                 single_2b, single_3, roi_contrast) {
  n_runners <- ranking_metrics_2b$n_runners[
    ranking_metrics_2b$metric == "Brier_place"]
  n_races <- unique(boot_ranking$n_races)
  stopifnot(length(n_races) == 1L)

  rank_rows <- boot_ranking |>
    dplyr::transmute(
      measure = metric,
      n_basis = dplyr::if_else(metric == "Brier_place",
                               sprintf("%s runner-rows", format(n_runners, big.mark = ",")),
                               sprintf("%s races", format(n_races, big.mark = ","))),
      point_2b = point_b, point_3 = point_a,
      diff_point, ci_lo, ci_hi
    )

  roi_row <- tibble::tibble(
    measure = "single-bet win ROI",
    n_basis = sprintf("%s / %s bets",
                      format(single_2b$n_bets, big.mark = ","),
                      format(single_3$n_bets, big.mark = ",")),
    point_2b = single_2b$roi, point_3 = single_3$roi,
    diff_point = roi_contrast$diff_point,
    ci_lo = roi_contrast$ci_lo, ci_hi = roi_contrast$ci_hi
  )

  dplyr::bind_rows(rank_rows, roi_row) |>
    dplyr::mutate(
      excludes_zero = ci_lo > 0 | ci_hi < 0,
      ci_width = ci_hi - ci_lo,
      # The interval width as a share of the quantity being measured. This is
      # the comparison the section turns on and it needs one denominator
      # across all four rows: the width relative to the point value each
      # measure takes. Dividing by the 2b-3 difference instead would compare
      # each measure against its own noise and say nothing about resolution.
      width_over_value = ci_width / abs(point_2b)
    )
}
